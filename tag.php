        ...
        <div class="container">
            ...
            <div class="thread-container">
                <div class="flt-left thread-parent">
                    <div class="pure-g">
                        <?php
                        $result = mysql_query("SELECT distinct category.name, useraccounts.username, extendedinfo.avatarpath, extendedinfo.fname, extendedinfo.lname, thread.srno,thread.title, thread.description, thread.imagepath, thread.coordinates, thread.timestamp, thread.uid FROM useraccounts, extendedinfo, thread, thread_tags, tags, category WHERE thread.cid = category.srno and thread_tags.tid = thread.srno and thread_tags.name = '".$tag."' and extendedinfo.uid = thread.uid and useraccounts.srno = thread.uid ORDER BY timestamp DESC") or die(mysql_error());                        
                        if (mysql_num_rows($result)) {
                            echo '<h4 class="light" style="padding: 10px 0;">Threads tagged as <b>' . $tag . '</b></h4>';
                                ...
                                echo'
                                        <div class="pure-u-1" style="position: relative;">
                                            ...
                                            if($row['uid']==$_SESSION['userid']){
                                                echo '<p class="bold thread-owner-link"><a rel="external" href="profile.php?username=' . $row['username'] . '">You</a></p>';
                                            }
                                            else{
                                                echo '<p class="bold thread-owner-link"><a rel="external" href="profile.php?username=' . $row['username'] . '">' . $row['fname'] . ' ' . $row['lname'] . '</a></p>';
                                            }
                            ...
                            <div class="pure-u-1 bg-white block-flat load-more-post-box" style="margin: 0 0 20px 0;padding: 10px;">
                                <p class="margin0 txt-center"><button data-tag="'.$_REQUEST['t'].'" class="load-more-post-tag btn-general bg-white fg-gray">LOAD MORE POSTS</button></p>
                            </div>';
                        } 
                        else {
                            echo '<h4 class="light" style="padding: 10px 0;">We couldn\'t find any threads :(</h4>';
                        }
                        ?>
                    </div>
                </div>                
